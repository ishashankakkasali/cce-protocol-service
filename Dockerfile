...
COPY cce-protocol-service/gradle/ cce-protocol-service/gradle/
COPY cce-protocol-service/gradlew cce-protocol-service/build.gradle cce-protocol-service/settings.gradle cce-protocol-service/

WORKDIR /workspace/cce-protocol-service
RUN chmod +x gradlew ../cce-common-util/gradlew \
    && ./gradlew dependencies --no-daemon 2>/dev/null || true

COPY cce-protocol-service/src/ src/
...
# The application's own default is 8090; pin it to the port this image EXPOSEs and
